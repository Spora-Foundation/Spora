...
use spora_wallet_core::{
    account::{Account, BIP32_ACCOUNT_KIND, MULTISIG_ACCOUNT_KIND, WATCH_ONLY_ACCOUNT_KIND, BIP32_WATCH_ACCOUNT_KIND},
...
async fn test_bip32watch_account_creation() {
    ...
    let account_args = AccountCreateArgsBip32Watch::new(
        Some("test-bip32watch".to_string()),
        vec![test_data.xpub_string.clone()],
    );
...
async fn test_multisig_account_creation() {
    ...
    let prv_key_data_args2 = PrvKeyDataCreateArgs::new(
        None,
        None,
        mnemonic2,
        PrvKeyDataVariantKind::Mnemonic,
    ...
    let prv_key_data_args = vec![
        PrvKeyDataArgs::new(prv_key_data_id1, None),
        PrvKeyDataArgs::new(prv_key_data_id2, None),
    ];
    
    let result = wallet.create_account_multisig(
        &wallet_secret,
        prv_key_data_args,
...
async fn test_account_lifecycle_integration() {
    ...
    assert_ne!(receive_address.to_string(), change_address.to_string(), 
        "receive and change addresses should be different");
...
async fn test_account_creation_error_handling() {
    ...
    let account_args = AccountCreateArgsWatchOnly::new(
        ...
        vec!["invalid_xpub_string".to_string()],
        1,
        false,
    );
