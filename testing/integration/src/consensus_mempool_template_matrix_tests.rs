...
impl IntegrationTestContext {
    async fn new() -> Self {
        ...
        let config = ConfigBuilder::new(network_id.into()).adjust_testnet_params().build();

        let consensus = TestConsensusFactory::create_temp(config.clone()).await;

        let counters = Arc::new(MiningCounters::default());
        let mining_manager =
            Arc::new(MiningManager::new(config.params.target_time_per_block(), false, config.params.max_block_mass, None, counters));

        Self { consensus: Arc::new(consensus), mining_manager, network_id }
    }
    ...
    fn create_cell_output(&self, capacity: u64, lock_script: Script) -> CellOutput {
        CellOutput { capacity, lock: lock_script, type_: None }
    }
...
async fn test_normal_transaction_flow() {
    ...
    let result = ctx
        .mining_manager
        .validate_and_insert_cell_transaction_batch(
            ctx.consensus_api(),
            vec![tx.clone()],
            Priority::Low,
            Orphan::Forbidden,
    ...
    assert!(
        !template.tx.is_empty() || template.block.header.hash != Hash::default(),
...
async fn test_consensus_rejected_tx_not_in_mempool() {
    ...
    let invalid_tx = CellTx::new(
        vec![CellInput::new(TransactionOutpoint::new(Hash::from_bytes([0xFFu8; 32]).as_bytes(), 0), 0)],
        vec![],
...
fn create_test_transaction_with_fee(ctx: &IntegrationTestContext, fee: u64, nonce: u32) -> CellTx {
    ...
    let prev_out = TransactionOutpoint::new(Hash::from_bytes(hash_bytes).as_bytes(), (fee % 10) as u32);
...
async fn test_batch_transaction_submission() {
    ...
    // 验证计数一致性
    assert_eq!(final_count, ready_txs.len() + orphan_txs.len(), "Transaction count should match sum of ready and orphan txs");
}
...
async fn test_mempool_eviction_policy() {
    ...
    // 验证高优先级交易被接受
    assert!(count_after_high >= count_after_low || count_after_high > 0, "High priority transaction should be accepted");
}
