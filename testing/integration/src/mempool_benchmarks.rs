...
async fn bench_bbt_latency() {
    tondi_core::log::try_init_logger("info,tondi_core::time=debug,tondi_mining::monitor=debug");
    ...
    */
    ...
    let submit_block_pool_tasks = submit_block_pool.start(|c, block| async move {
        let _sw = tondi_core::time::Stopwatch::<500>::with_threshold("sb");
    ...
    let submit_tx_pool_tasks = submit_tx_pool.start(|c, (i, tx)| async move {
        match c.submit_transaction(tx.as_ref().into(), false).await {
            ...
            Err(RpcError::General(msg)) if msg.contains("orphan") => {
                tondi_core::warn!("\n\n\n{msg}\n\n");
                tondi_core::warn!("Submitted {} transactions, exiting tx submit loop", i);
                return true;
            }
            Err(e) => panic!("{e}"),
        }
        ...
        while let Ok(notification) = notification_rx.recv().await {
            match notification {
                Notification::NewBlockTemplate(_) => {
                    ...
                    // let _sw = tondi_core::time::Stopwatch::<500>::with_threshold("bbt");
                    *current_template.lock() = cc.get_block_template(pac.clone(), vec![]).await.unwrap();
                }
                _ => panic!(),
            }
            if !exec.load(Ordering::Relaxed) {
                tondi_core::warn!("Test is over, stopping miner receiver loop");
                break;
            }
        }
        tondi_core::warn!("Miner receiver loop task exited");
    });
    ...
    let miner_loop_task = tokio::spawn(async move {
        for i in 0..BLOCK_COUNT {
            ...
            if !exec.load(Ordering::Relaxed) {
                tondi_core::warn!("Test is over, stopping miner loop");
                break;
            }
        }
        ...
        tondi_core::warn!("Miner loop task exited");
    });
    ...
    let tx_sender_task = tokio::spawn(async move {
        ...
        exec.store(false, Ordering::Relaxed);
        tondi_core::warn!("Tx sender task exited");
    });
...
/// Run this benchmark with the following command line:
/// `cargo test --release --package tondi-testing-integration --lib --features devnet-prealloc -- mempool_benchmarks::bench_bbt_latency_2 --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
