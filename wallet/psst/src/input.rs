...
impl Add for Input {
    ...
    fn add(mut self, rhs: Self) -> Self::Output {
        if self.previous_outpoint.tx_hash != rhs.previous_outpoint.tx_hash {
            return Err(CombineError::PreviousTxidMismatch {
                this: TransactionId::from_bytes(self.previous_outpoint.tx_hash),
                that: TransactionId::from_bytes(rhs.previous_outpoint.tx_hash),
            });
        }
        ...
        self.cell_entry = match (self.cell_entry.take(), rhs.cell_entry) {
            (None, None) => None,
