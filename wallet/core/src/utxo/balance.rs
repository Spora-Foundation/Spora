...
export interface IBalance {
    /**
     * Total amount of Tondi (in SOMPI) available for 
     * spending.
     */
    mature: bigint;
    /**
     * Total amount of Tondi (in SOMPI) that has been 
     * received and is pending confirmation.
     */
    pending: bigint;
    /**
     * Total amount of Tondi (in SOMPI) currently 
...
impl From<(Option<&Balance>, &NetworkType, Option<usize>)> for BalanceStrings {
    fn from((balance, network_type, padding): (Option<&Balance>, &NetworkType, Option<usize>)) -> Self {
        ...
        if let Some(balance) = balance {
            let mut mature = utils::sompi_to_tondi_string(balance.mature);
            let mut pending = if balance.pending > 0 { Some(utils::sompi_to_tondi_string(balance.pending)) } else { None };
