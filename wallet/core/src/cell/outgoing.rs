...
struct Inner {
    ...
    pub originating_context: CellContext,
    pub destination_context: Option<CellContext>,
    #[allow(dead_code)]
...
/// convenience methods for handling within [`CellContext`].
#[derive(Clone)]
...
impl OutgoingTransaction {
    pub fn new(current_daa_score: u64, originating_context: CellContext, pending_transaction: PendingTransaction) -> Self {
        let destination_context = pending_transaction.generator().destination_cell_context().clone();
    ...
    pub fn cell_entries(&self) -> &AHashMap<CellEntryId, CellEntryReference> {
        self.inner.pending_transaction.cell_entries()
    }

    pub fn originating_context(&self) -> &CellContext {
        &self.inner.originating_context
    }

    pub fn destination_context(&self) -> &Option<CellContext> {
