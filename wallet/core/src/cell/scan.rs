...
//! aggregating tracked cells from multiple addresses and
//! building corresponding balances.
//!

use crate::cell::balance::AtomicBalance;
use crate::cell::{CellContext, CellEntryReference, CellEntryReferenceExtension};
...
impl Scan {
    ...
    pub async fn scan(&self, cell_context: &CellContext) -> Result<()> {
        // block notifications while scanning...
        let _lock = cell_context.processor().notification_lock().await;

        match &self.provider {
            Provider::AddressManager(address_manager) => self.scan_with_address_manager(address_manager, cell_context).await,
            Provider::AddressSet(addresses) => self.scan_with_address_set(addresses, cell_context).await,
        }
    }

    pub async fn scan_cells(&self, cell_context: &CellContext) -> Result<()> {
        self.scan(cell_context).await
    }

    pub async fn scan_with_address_manager(&self, address_manager: &Arc<AddressManager>, cell_context: &CellContext) -> Result<()> {
        let params = cell_context.processor().network_params()?;
        ...
        'scan: loop {
            ...
            // register addresses in the tracked cell context; NOTE: during the scan,
            // before `get_cells_by_addresses()` is complete we may receive
            // new transactions as such the cell context should be aware of the
            // addresses used before we start interacting with them.
            cell_context.register_addresses(&addresses).await?;

            let ts = Instant::now();
            let resp = cell_context.processor().rpc_api().get_cells_by_addresses(addresses).await?;
            let elapsed_sec = ts.elapsed().as_secs_f32();
            if elapsed_sec > 1.0 {
                log_warn!("get_cells_by_addresses() fetched {} entries in: {} msec", resp.len(), elapsed_sec);
            }
            yield_executor().await;

            if !resp.is_empty() {
                let refs: Vec<CellEntryReference> = resp.into_iter().map(CellEntryReference::from).collect();
                for cell_ref in refs.iter() {
                    if let Some(address) = cell_ref.cell.address.as_ref() {
                        if let Some(cell_address_index) = address_manager.inner().address_to_index_map.get(address) {
                            if last_address_index < *cell_address_index {
                                last_address_index = *cell_address_index;
                            }
                        } else {
                ...
                let balance: Balance = refs.iter().fold(Balance::default(), |mut balance, r| {
                    ...
                    balance.mature_cell_count += entry_balance.mature_cell_count;
                    balance.pending_cell_count += entry_balance.pending_cell_count;
                    balance.stasis_cell_count += entry_balance.stasis_cell_count;
                    balance
                });

                cell_context.extend_from_scan(refs, self.current_daa_score).await?;
    ...
    pub async fn scan_with_address_set(&self, address_set: &HashSet<Address>, cell_context: &CellContext) -> Result<()> {
        let params = cell_context.processor().network_params()?;
        let address_vec = address_set.iter().cloned().collect::<Vec<_>>();

        cell_context.register_addresses(&address_vec).await?;
        let resp = cell_context.processor().rpc_api().get_cells_by_addresses(address_vec).await?;
        let refs: Vec<CellEntryReference> = resp.into_iter().map(CellEntryReference::from).collect();
