//!
//! Implements [`CellProcessor`], which is the main component
//! of the wallet cell-tracking subsystem. It is responsible for managing and
//! coordinating multiple [`CellContext`] instances acting as
//! a hub for cell event dispersal and related processing.
//!
...
use spora_notify::{
    listener::ListenerId,
    scope::{CellsChangedScope, Scope, VirtualDaaScoreChangedScope},
};
use spora_rpc_core::{
    api::{
        ctl::{RpcCtl, RpcState},
    ...
    message::CellsChangedNotification,
    GetServerInfoResponse, RpcFeeEstimate,
};
use spora_wrpc_client::SporaRpcClient;
use workflow_core::channel::{Channel, DuplexChannel, Sender};
use workflow_core::task::spawn;

use crate::cell::{CellContext, CellEntryId, Maturity, OutgoingTransaction, PendingCellEntryReference, SyncMonitor};
...
pub struct Inner {
    /// Coinbase cells in stasis
    stasis: DashMap<CellEntryId, PendingCellEntryReference>,
    /// Cells pending maturity
    pending: DashMap<CellEntryId, PendingCellEntryReference>,
    /// Outgoing Transactions
    outgoing: DashMap<TransactionId, OutgoingTransaction>,
    /// Address to CellContext map (maps all addresses used by
    /// all CellContexts to their respective CellContexts)
    address_to_cell_context_map: DashMap<Arc<Address>, CellContext>,
    // ---
...
impl Inner {
    pub fn new(
        ...
        Self {
            ...
            address_to_cell_context_map: DashMap::new(),
...
impl CellProcessor {
    pub fn new(
        ...
        CellProcessor { inner: Arc::new(Inner::new(rpc, network_id, multiplexer, wallet_bus)) }
    }

    pub fn rpc_api(&self) -> Arc<DynRpcApi> {
        self.inner.rpc.lock().unwrap().as_ref().expect("CellProcessor RPC not initialized").rpc_api().clone()
    }
    ...
    pub fn rpc_ctl(&self) -> RpcCtl {
        self.inner.rpc.lock().unwrap().as_ref().expect("CellProcessor RPC not initialized").rpc_ctl().clone()
    }
    ...
    pub fn rpc_client(&self) -> Option<Arc<SporaRpcClient>> {
        self.rpc_api().clone().downcast_arc::<SporaRpcClient>().ok()
    }
    ...
    pub fn pending(&self) -> &DashMap<CellEntryId, PendingCellEntryReference> {
        &self.inner.pending
    }
    ...
    pub fn stasis(&self) -> &DashMap<CellEntryId, PendingCellEntryReference> {
        &self.inner.stasis
    }
    ...
    pub fn address_to_cell_context_map(&self) -> &DashMap<Arc<Address>, CellContext> {
        &self.inner.address_to_cell_context_map
    }

    pub fn address_to_cell_context(&self, address: &Address) -> Option<CellContext> {
        self.inner.address_to_cell_context_map.get(address).map(|v| v.clone())
    }

    pub async fn register_addresses(&self, addresses: Vec<Arc<Address>>, cell_context: &CellContext) -> Result<()> {
        addresses.iter().for_each(|address| {
            self.inner.address_to_cell_context_map.insert(address.clone(), cell_context.clone());
        });

        if self.is_connected() {
            if !addresses.is_empty() {
                let addresses = addresses.into_iter().map(|address| (*address).clone()).collect::<Vec<_>>();
                let cells_changed_scope = CellsChangedScope::new(addresses);
                self.rpc_api().start_notify(self.listener_id()?, cells_changed_scope.into()).await?;
            } else {
    ...
    pub async fn unregister_addresses(&self, addresses: Vec<Arc<Address>>) -> Result<()> {
        addresses.iter().for_each(|address| {
            self.inner.address_to_cell_context_map.remove(address);
        });
        ...
        if self.is_connected() {
            if !addresses.is_empty() {
                let addresses = addresses.into_iter().map(|address| (*address).clone()).collect::<Vec<_>>();
                let cells_changed_scope = CellsChangedScope::new(addresses);
                self.rpc_api().stop_notify(self.listener_id()?, cells_changed_scope.into()).await?;
            } else {
    ...
    pub async fn handle_pending(&self, current_daa_score: u64) -> Result<()> {
        ...
        let promotions =
            HashMap::group_from(mature_entries.into_iter().map(|cell| (cell.inner.cell_context.clone(), cell.inner.entry.clone())));
        let mut updated_contexts: HashSet<CellContext> = HashSet::from_iter(promotions.keys().cloned());

        for (context, cells) in promotions.into_iter() {
            context.promote(cells).await?;
        }

        // ------

        let revivals =
            HashMap::group_from(revived_entries.into_iter().map(|cell| (cell.inner.cell_context.clone(), cell.inner.entry.clone())));
        updated_contexts.extend(revivals.keys().cloned());

        for (context, cells) in revivals.into_iter() {
            context.revive(cells).await?;
        }
    ...
    pub async fn handle_discovery(&self, record: TransactionRecord) -> Result<()> {
        if let Some(wallet_bus) = self.wallet_bus() {
            // if CellProcessor has an associated wallet_bus installed
    ...
    pub async fn handle_cells_changed(&self, cells: CellsChangedNotification) -> Result<()> {
        let current_daa_score = self.current_daa_score().expect("DAA score expected when handling cells-changed notifications");

        #[allow(clippy::mutable_key_type)]
        let mut updated_contexts: HashSet<CellContext> = HashSet::default();

        let added = (*cells.added).clone().into_iter().filter_map(|entry| entry.address.clone().map(|address| (address, entry)));
        let mut added = HashMap::group_from(added);

        let removed = (*cells.removed).clone().into_iter().filter_map(|entry| entry.address.clone().map(|address| (address, entry)));
        ...
        for (address, entries) in removed.into_iter() {
            if let Some(cell_context) = self.address_to_cell_context(&address) {
                updated_contexts.insert(cell_context.clone());
                let entries = entries.iter().map(|entry| entry.into()).collect::<Vec<_>>();
                if entries.is_not_empty() {
                    cell_context.handle_cell_removed(entries, current_daa_score).await?;
                }
            } else {
                log_error!("receiving cells-changed 'removed' notification for an unknown address: {}", address);
            }
        }
        ...
        for (address, entries) in added.into_iter() {
            if let Some(cell_context) = self.address_to_cell_context(&address) {
                ...
                let entries = entries.iter().map(|entry| entry.into()).collect::<Vec<_>>();
                if entries.is_not_empty() {
                    cell_context.handle_cell_added(entries, current_daa_score).await?;
                }
            } else {
                log_error!("receiving cells-changed 'added' notification for an unknown address: {}", address);
            }
        }
        ...
        for (address, entries_added) in common_added.into_iter() {
            if let Some(cell_context) = self.address_to_cell_context(&address) {
                ...
                let added_cells = entries_added.iter().map(|entry| entry.into()).collect::<Vec<_>>();
                //let removed_cells = entries_removed.iter().map(|entry| entry.into()).collect::<Vec<_>>();

                cell_context.update_cells(added_cells, current_daa_score).await?;
            } else {
                log_error!("receiving cells-changed 'added' notification for an unknown address: {}", address);
            }
        }
    ...
    pub async fn register_cell_addresses(&self, addresses: Vec<Arc<Address>>, cell_context: &CellContext) -> Result<()> {
        self.register_addresses(addresses, cell_context).await
    }

    pub async fn unregister_cell_addresses(&self, addresses: Vec<Arc<Address>>) -> Result<()> {
        self.unregister_addresses(addresses).await
    }
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        if !has_cell_index {
            self.notify(Events::CellIndexNotEnabled { url: self.rpc_url() }).await?;
            return Err(Error::MissingCellIndex);
        }
        ...
        log_trace!("Connected to Sporad: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}");
    ...
    pub async fn handle_connect_impl(&self) -> Result<()> {
        ...
        self.notify(Events::CellProcStart).await?;
        ...
        match self.handle_connect_impl().await {
            Err(err) => {
                if !self.signal_connection(Err(err.to_string())) {
                    log_error!("CellProcessor: error while connecting to node: {err}");
                }
                self.notify(Events::CellProcError { message: err.to_string() }).await?;
    ...
    pub async fn handle_disconnect(&self) -> Result<()> {
        ...
        self.notify(Events::CellProcStop).await?;
        self.cleanup().await?;
    ...
    pub async fn cleanup(&self) -> Result<()> {
        ...
        self.inner.address_to_cell_context_map.clear();
        Ok(())
    }
    ...
    async fn handle_notification(&self, notification: Notification) -> Result<()> {
        ...
        match notification {
            ...
            Notification::CellsChanged(cells_changed_notification) => {
                if !self.is_synced() {
                    self.sync_proc().track(true).await?;
                }

                self.handle_cells_changed(cells_changed_notification).await?;
            }
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        if this.inner.task_is_running.load(Ordering::SeqCst) {
            return Err(Error::custom("CellProcessor::start() called while task is already running"));
        }
        ...
        // clients relying on CellProcessor state should monitor
        // for and handle `CellProcStart` and `CellProcStop` events.
                ...
                select_biased! {
                    msg = rpc_ctl_channel.receiver.recv().fuse() => {
                        match msg {
                            ...
                            Err(err) => {
                                log_error!("CellProcessor: error while receiving rpc_ctl_channel message: {err}");
                                log_error!("Suspending cell processor...");
                                break;
                            }
                        }
                    }
                        ...
                        match notification {
                            Ok(notification) => {
                                if let Err(err) = this.handle_notification(notification).await {
                                    this.notify(Events::CellProcError { message: err.to_string() }).await.ok();
    ...
    pub async fn stop(&self) -> Result<()> {
        if self.inner.task_is_running.load(Ordering::SeqCst) {
            ...
            self.inner.task_ctl.signal(()).await.expect("CellProcessor::stop_task() `signal` error");
        }
        Ok(())
    }
    ...
    pub async fn stop_fee_rate_poller(&self) -> Result<()> {
        if self.inner.fee_rate_task_is_running.load(Ordering::SeqCst) {
            self.inner.fee_rate_task_ctl.signal(()).await.expect("CellProcessor::stop_task() `signal` error");
        }
        Ok(())
    }
}
