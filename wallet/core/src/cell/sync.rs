...
impl Default for StateObserver {
    fn default() -> Self {
        Self {
            ...
            cell_resync: Regex::new(r"Resyncing the cellindex\.\.\.").unwrap(),
            cell_sync: Regex::new(r"Received (\d+) cell set chunks so far, totaling in (\d+) cells").unwrap(),
...
impl StateObserver {
    pub fn get(&self, line: &str) -> Option<SyncState> {
        ...
        if let Some(captures) = self.ibd_headers.captures(line) {
        ...
        } else if let Some(captures) = self.cell_sync.captures(line) {
            if let (Some(chunks), Some(total)) = (captures.get(1), captures.get(2)) {
                if let (Ok(chunks), Ok(total)) = (chunks.as_str().parse::<u64>(), total.as_str().parse::<u64>()) {
                    state = Some(SyncState::CellSync { chunks, total });
                }
            }
        ...
        } else if self.cell_resync.is_match(line) {
            state = Some(SyncState::CellResync);
        }
