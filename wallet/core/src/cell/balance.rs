...
/**
 * Tracked cell context (wallet account) balance.
 * @category Wallet SDK
 */
export interface IBalance {
    ...
    outgoing: bigint;
    /**
     * Number of spendable cells.
     */
    matureCellCount: number;
    /**
     * Number of cells that have been received and 
     * are pending confirmation.
     */
    pendingCellCount: number;
    /**
     * Number of cells currently in stasis (coinbase 
...
pub struct Balance {
    ...
    pub pending: u64,
    pub outgoing: u64,
    pub mature_cell_count: usize,
    pub pending_cell_count: usize,
    pub stasis_cell_count: usize,
    #[serde(skip)]
    mature_delta: Delta,
    #[serde(skip)]
    pending_delta: Delta,
...
impl Balance {
    pub fn new(
        ...
        pending: u64,
        outgoing: u64,
        mature_cell_count: usize,
        pending_cell_count: usize,
        stasis_cell_count: usize,
    ) -> Self {
        Self {
            mature,
            pending,
...
pub struct AtomicBalance {
    ...
    pub mature_cells: AtomicUsize,
    pub pending_cells: AtomicUsize,
    pub stasis_cells: AtomicUsize,
}

pub type CellBalance = Balance;
pub type AtomicCellBalance = AtomicBalance;
...
impl AtomicBalance {
    pub fn add(&self, balance: Balance) {
        ...
        self.mature_cells.fetch_add(balance.mature_cell_count, Ordering::SeqCst);
        self.pending_cells.fetch_add(balance.pending_cell_count, Ordering::SeqCst);
        self.stasis_cells.fetch_add(balance.stasis_cell_count, Ordering::SeqCst);
    }
}
...
impl From<AtomicBalance> for Balance {
    fn from(atomic_balance: AtomicBalance) -> Self {
        Self {
            ...
            outgoing: 0,
            mature_cell_count: atomic_balance.mature_cells.load(Ordering::SeqCst),
            pending_cell_count: atomic_balance.pending_cells.load(Ordering::SeqCst),
            stasis_cell_count: atomic_balance.stasis_cells.load(Ordering::SeqCst),
            mature_delta: Delta::default(),
            pending_delta: Delta::default(),
        }
    }
