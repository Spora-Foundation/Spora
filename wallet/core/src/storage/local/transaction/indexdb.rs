    ...
    async fn open_db(&self, db_name: String) -> Result<IdbDatabase> {
        call_async_no_send!(async move {
            ...
            let on_upgrade_needed = move |evt: &IdbVersionChangeEvent| -> Result<(), JsValue> {
                let db = evt.db();
                if !db.object_store_names().any(|name| name == TRANSACTIONS_STORE_NAME) {
                    let object_store = db.create_object_store(TRANSACTIONS_STORE_NAME)?;
...
impl TransactionStore {
    pub fn new(_name: &str) -> TransactionStore {
        TransactionStore { inner: Arc::new(Mutex::new(Arc::new(Inner))) /* name: name.to_string(), */ }
    }
