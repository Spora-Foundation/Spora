...
impl PrivateKey {
    ...
    /// Receives a [`NetworkType`](spora_consensus_core::network::NetworkType)
    ...
    pub fn to_address(&self, network: &NetworkTypeT) -> Result<Address> {
        ...
        Ok(Address::new_std_single(network.try_into()?, &x_only_public_key.serialize())?)
    }
    ...
    pub fn to_address_ecdsa(&self, network: &NetworkTypeT) -> Result<Address> {
        let public_key = secp256k1::PublicKey::from_secret_key_global(&self.inner);
        Ok(Address::new_std_single_ecdsa(network.try_into()?, &public_key.serialize())?)
    }
}
