    ...
    SampledWindowManager<T, U, V, W>
{
    ...
    pub(crate) fn is_window_block_activated(&self, _selected_parent: Hash) -> bool {
        true
    }
    ...
    fn build_block_window(
        ...
        // Filter non-activated blocks from the window
        let filter_non_activated = true;

        /*
            Extended explanation

            For the window we filter all previously non-activated blocks.
        ...
        */
        ...
        if let Some(res) = self.try_init_from_cache(
            ...
            // The selected parent window will be in the cache only if it was activated (due to tracking of window origin),
        ...
        loop {
            ...
            // The chain ancestor window will be in the cache only if it was
        ...
        once(selected_parent_block)
            ...
            .filter_map(move |block| {
                ...
                if compact.blue_score < blue_score_threshold {
                    ...
                    index += 1;
                    if filter_non_activated && !self.is_window_block_activated(compact.selected_parent) {
                        return None;
                    }
        ...
        while !unvisited.is_empty() {
            ...
            for merged in ghostdag.unordered_mergeset() {
                ...
                if unvisited.remove(&merged) {
                    // For each block in the original window save its selected parent as well
    ...
    DualWindowManager<T, U, V, W>
{
    pub fn new(
        ...
        Self { ghostdag_store, headers_store, sampled_window_manager, full_window_manager }
    }

    /// Sampling is always active
    pub(crate) fn sampling(&self, _selected_parent: Hash) -> bool {
        true
    }
}
