...
pub fn validate_cellbase_maturity<P: DagCellProvider>(
    ...
    for input in &tx.inputs {
        // Get Cell metadata
        let meta = provider
            .get_cell_metadata(&input.out_point)
...
pub fn validate_cell_existence<P: DagCellProvider>(tx: &CellTx, current_daa: u64, provider: &P) -> Result<(), CellValidationError> {
    // Check all inputs exist
    for input in &tx.inputs {
        let available =
            provider.is_cell_available(&input.out_point, current_daa).map_err(|e| CellValidationError::InvalidFormat(e))?;
    ...
    for dep in &tx.deps {
        let available = provider.is_cell_available(&dep.out_point, current_daa).map_err(|e| CellValidationError::InvalidFormat(e))?;
...
pub fn validate_in_reorg_context<P: DagCellProvider>(tx: &CellTx, block_daa: u64, provider: &P) -> Result<(), CellValidationError> {
