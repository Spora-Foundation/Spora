...
#[cfg(feature = "vm")]
use spora_exec::vm::{ScriptError, SimpleDataProvider, TransactionScriptVerifier};
...
impl<P: CellStateProvider> CellValidator<P> {
    ...
    /// - Capacity conservation
    /// - Time locks (since field)
    pub fn validate_in_context(&self, tx: &spora_exec::CellTx, daa_score: u64) -> Result<(), CellValidationError> {
        cell_validation_in_context::validate_cell_tx_in_context(tx, daa_score, self.provider.as_ref())
    }
    ...
    pub fn validate_in_dag(&self, tx: &spora_exec::CellTx, daa_score: u64) -> Result<(), CellValidationError>
    where
        P: cell_validation_in_dag::DagCellProvider,
    {
        ...
        cell_validation_in_dag::validate_cellbase_maturity(tx, daa_score, self.params.cellbase_maturity, self.provider.as_ref())
    }

    /// Full validation (isolation + context + DAG)
    pub fn validate_full(&self, tx: &spora_exec::CellTx, daa_score: u64) -> Result<(), CellValidationError>
    where
        P: cell_validation_in_dag::DagCellProvider,
    {
    ...
    #[cfg(feature = "vm")]
    pub fn verify_scripts(&self, tx: &spora_exec::CellTx) -> Result<(), CellValidationError> {
        // Create data provider
        // TODO: Use real data provider from consensus storage
        let provider = Arc::new(SimpleDataProvider::new());

        // Create verifier
        let verifier = TransactionScriptVerifier::new(Arc::new(tx.clone()), provider);

        // Verify all scripts
        verifier.verify().map_err(|e| CellValidationError::ScriptVerificationFailed(e.to_string()))?;
    ...
    #[cfg(feature = "vm")]
    pub fn validate_full_with_scripts(&self, tx: &spora_exec::CellTx, daa_score: u64) -> Result<(), CellValidationError>
    where
        P: cell_validation_in_dag::DagCellProvider,
    {
        // Standard validation
