...
impl CoinbaseManager {
    pub fn new(
        ...
        // In a 10 BPS network, the induced increase in total rewards is 51 SPORA (see tests::calc_high_bps_total_rewards_delta())
       ...
       10986328,    10369712,     9787705,     9238363,     8719854,     8230446,     7768507,     7332494,     6920953,     6532509,     6165868,     5819804,
        ...
        1373291,     1296214,     1223463,     1154795,     1089981,     1028805,      971063,      916561,      865119,      816563,      770733,      727475,
         ...
         171661,      162026,      152932,      144349,      136247,      128600,      121382,      114570,      108139,      102070,       96341,       90934,
          ...
          21457,       20253,       19116,       18043,       17030,       16075,       15172,       14321,       13517,       12758,       12042,       11366,
...
mod tests {
    ...
    use spora_consensus_core::{
        config::params::{ForkActivation, Params, SIMNET_PARAMS, TESTNET_PARAMS},
    ...
    fn test_calc_high_bps_total_rewards() {
        ...
        let base_subsidy = 450_00000000f64;
        let mut month = 0f64;

        let mut subsidy_table = vec![];
        loop {
            let subsidy = base_subsidy / 2f64.powf(month / 12f64);
            subsidy_table.push(subsidy as u64);
            if subsidy as u64 == 0 {
                break;
            }
            month += 1f64;
        ...
        let total_rewards: u64 = pre_deflationary_rewards + subsidy_table.iter().map(|x| x * SECONDS_PER_MONTH).sum::<u64>();
        let bps = TESTNET_PARAMS.bps().upper_bound();
        let total_high_bps_rewards_rounded_up: u64 =
            pre_deflationary_rewards + subsidy_table.iter().map(|x| (x.div_ceil(bps) * bps) * SECONDS_PER_MONTH).sum::<u64>();
        ...
        println!("Subsidy Table :{subsidy_table:?}");
        println!("Pre Deflationary Rewards :{pre_deflationary_rewards}");
        println!("Total rewards: {} sau => {} SPORA", total_rewards, total_rewards / SAU_PER_TONDI);
        println!("Total high bps rewards: {} sau => {} SPORA", total_high_bps_rewards, total_high_bps_rewards / SAU_PER_TONDI);
        println!("Delta: {} sau => {} SPORA", delta, delta / SAU_PER_TONDI as i64);
    }
    ...
    /// `cargo test --release --package spora-consensus --lib -- processes::coinbase::tests::verify_crescendo_emission_schedule --exact --nocapture --ignored`
    #[test]
    #[ignore = "long"]
