...
//
// Reference tests for MPE parallelization.
//
...
// against the 5 protocol semantic rules defined in MPE_PROTOCOL_SEMANTICS.md.
//
...
mod tests {
    ...
    use crate::pipeline::virtual_processor::{
        ...
        cell_processing::{apply_cell_diff_to_tree, execution_effect_conflicts_with_current_state, BlockExecutionEffect},
    ...
    // ========================================================================
    //
    // Verifies MPE_PROTOCOL_SEMANTICS invariant 1: for the same mergeset,
    ...
    fn test_same_mergeset_deterministic_dag_layering() {
        ...
        assert_eq!(dag1.layers.len(), 1, "All independent blocks should be in a single layer");
    ...
    // ========================================================================
    //
    // Verifies MPE_PROTOCOL_SEMANTICS rule 1: when the same tx_id appears
    ...
    fn test_duplicate_tx_detected_by_access_summary() {
        ...
        assert_eq!(dag.layers.len(), 2, "Duplicate tx blocks must be in separate layers");
    ...
    // ========================================================================
    //
    // Verifies MPE_PROTOCOL_SEMANTICS rule 2: when different transactions in
    ...
    fn test_double_spend_detected_by_access_summary() {
        ...
        let dag = ExecutionDAG::build(&[summary_a, summary_b]);
        assert_eq!(dag.layers.len(), 2, "Double-spend blocks must be serialized");
    }
    ...
    fn test_double_spend_effect_invalidation_at_commit_time() {
        ...
        let effect_b = {
            ...
            effect.cell_diff = diff_b;
            effect
        };

        assert!(
            execution_effect_conflicts_with_current_state(&tree, &effect_b),
            "Conflict must be detected: contested cell was already consumed by A"
        );
    ...
    #[test]
    fn test_cell_dep_effect_invalidation_at_commit_time() {
        use spora_state::CellStateTree;

        let dep = outpoint(0xD0, 0);
        let dep_meta = make_cell_meta(dep, 5000, 1, true);

        let mut tree = CellStateTree::new();
        let dep_hash = crate::processes::utils::outpoint_to_hash(&dep);
        tree.insert_with_outpoint(
            dep_hash,
            spora_exec::OutPoint::new(dep.tx_hash, dep.index),
            spora_state::CellEntry::new(5000, 0, hash(1), None, hash(2), 1, true),
        );

        let mut spending_diff = CellDiff::default();
        spending_diff.remove_cell(dep, dep_meta);
        apply_cell_diff_to_tree(&mut tree, &spending_diff);

        let mut reader_effect = BlockExecutionEffect::empty(hash(0xB0), 10);
        reader_effect.read_deps.insert(dep);

        assert!(
            execution_effect_conflicts_with_current_state(&tree, &reader_effect),
            "Commit guard must invalidate an effect whose cell_dep was consumed by an earlier same-layer commit"
        );
    }
    ...
    fn test_cross_block_spend_dependency() {
        ...
        let dag = ExecutionDAG::build(&[summary_a, summary_b]);
        assert_eq!(dag.layers.len(), 2, "A→B dependency must put them in separate layers");
    ...
    fn test_cross_block_read_dependency() {
        ...
        let dag = ExecutionDAG::build(&[summary_a, summary_b]);
        assert_eq!(dag.layers.len(), 2, "Read dependency must serialize the blocks");
    }

    #[test]
    fn test_cross_block_read_after_spend_dependency() {
        // Block A consumes a live cell. Block B uses that same cell as a
        // read-only cell_dep. In serial execution B's validation runs after A
        // and must not be analyzed against the pre-A snapshot.
        let dep_spent_by_a = outpoint(0xA0, 0);
        let summary_a = make_summary(1, &[dep_spent_by_a], &[outpoint(0xA1, 0)], &[], &[hash(0x10)]);
        let summary_b = make_summary(2, &[outpoint(0xB1, 0)], &[outpoint(0xB0, 0)], &[dep_spent_by_a], &[hash(0x20)]);

        assert!(summary_b.has_dependency_on(&summary_a), "B must depend on A when B reads a cell A spent");

        let dag = ExecutionDAG::build(&[summary_a, summary_b]);
        assert_eq!(dag.layers.len(), 2, "Read-after-spend dependency must serialize the blocks");
    ...
    fn test_cross_block_dependency_with_independent_third_block() {
        ...
        // Expected: layer 0 = [A], layer 1 = [B, C]. C is independent, but
        // it stays behind B to preserve global canonical commit order.
        let created_by_a = outpoint(0xA0, 0);
        let summary_a = make_summary(1, &[], &[created_by_a], &[], &[hash(0x10)]);
        ...
        assert_eq!(dag.layers[0], vec![0], "A must be committed before B");
        assert_eq!(dag.layers[1], vec![1, 2], "B and C can share the post-A snapshot");
    }
    ...
    // ========================================================================
    //
    // Verifies MPE_PROTOCOL_SEMANTICS rules 4 and 5:
    ...
    // ========================================================================
    //
    // Verifies MPE_PROTOCOL_SEMANTICS invariant 1: serial and parallel
    ...
    fn test_serial_vs_parallel_equivalence_with_dependencies() {
        ...
        // MPE: layer 0 = [A], layer 1 = [B, C, D].
        // The layer partition preserves global canonical commit order exactly,
        // while still allowing B/C/D to share the post-A snapshot when they do
        ...
        let dag = ExecutionDAG::build(&summaries);
        assert_eq!(dag.layers.len(), 2);
        assert_eq!(dag.layers[0], vec![0], "Layer 0: A");
        assert_eq!(dag.layers[1], vec![1, 2, 3], "Layer 1: B, C, D");

        let parallel_commit_order: Vec<usize> = dag.layers.iter().flat_map(|l| l.iter().copied()).collect();
        assert_eq!(parallel_commit_order, vec![0, 1, 2, 3], "MPE commit order must remain canonical");
    }
    ...
    fn test_serial_vs_parallel_equivalence_full_scenario() {
        ...
        //
        // DAG: layer 0 = [A, B], layer 1 = [C, D]
        //
        // Serial order: A, B, C, D
        // Parallel commit: A, B (layer 0), then C, D (layer 1)
        //
        ...
        // --- Parallel processing (layer 0: A, B; layer 1: C, D) ---
        let mut parallel_merged = CellDiff::default();
    ...
    #[test]
    fn test_same_mergeset_deterministic_reference_outputs() {
        let created_by_a = outpoint(0xA0, 0);
        let summaries = vec![
            make_summary(1, &[], &[created_by_a], &[], &[hash(0x10)]),
            make_summary(2, &[outpoint(0xB0, 0)], &[outpoint(0xB1, 0)], &[], &[hash(0x20)]),
            make_summary(3, &[created_by_a], &[outpoint(0xC0, 0)], &[], &[hash(0x30)]),
            make_summary(4, &[outpoint(0xD0, 0)], &[outpoint(0xD1, 0)], &[], &[hash(0x40)]),
        ];

        let outputs = (0..3)
            .map(|_| {
                let dag = ExecutionDAG::build(&summaries);
                let commit_order = dag.layers.iter().flat_map(|layer| layer.iter().copied()).collect::<Vec<_>>();
                let accepted_ids =
                    commit_order.iter().map(|&idx| summaries[idx].tx_ids.iter().copied().next().unwrap()).collect::<Vec<_>>();
                (dag.layers, commit_order, accepted_ids)
            })
            .collect::<Vec<_>>();

        assert!(outputs.windows(2).all(|pair| pair[0] == pair[1]), "same mergeset must produce deterministic DAG outputs");
        assert_eq!(outputs[0].1, vec![0, 1, 2, 3], "commit order must remain canonical");
    }

    #[test]
    fn test_serial_vs_parallel_reference_runner_full_equivalence() {
        let cell_a = outpoint(0xA0, 0);
        let cell_b = outpoint(0xB0, 0);
        let cell_c = outpoint(0xC0, 0);
        let meta_a = make_cell_meta(cell_a, 5_000, 10, true);
        let meta_b = make_cell_meta(cell_b, 4_000, 11, false);
        let meta_c = make_cell_meta(cell_c, 3_000, 12, false);

        let mut diffs = Vec::new();
        let mut diff_a = CellDiff::default();
        diff_a.add_cell(cell_a, meta_a.clone());
        diffs.push(diff_a);

        let mut diff_b = CellDiff::default();
        diff_b.add_cell(cell_b, meta_b.clone());
        diffs.push(diff_b);

        let mut diff_c = CellDiff::default();
        diff_c.remove_cell(cell_a, meta_a);
        diff_c.add_cell(cell_c, meta_c);
        diffs.push(diff_c);

        let summaries = vec![
            make_summary(1, &[], &[cell_a], &[], &[hash(0x10)]),
            make_summary(2, &[outpoint(0xB9, 0)], &[cell_b], &[], &[hash(0x20)]),
            make_summary(3, &[cell_a], &[cell_c], &[], &[hash(0x30)]),
        ];
        let dag = ExecutionDAG::build(&summaries);

        let mut serial = CellDiff::default();
        for diff in &diffs {
            serial.with_diff_in_place(diff).unwrap();
        }

        let mut parallel = CellDiff::default();
        for idx in dag.layers.iter().flat_map(|layer| layer.iter().copied()) {
            parallel.with_diff_in_place(&diffs[idx]).unwrap();
        }

        assert_eq!(dag.layers.iter().flat_map(|layer| layer.iter().copied()).collect::<Vec<_>>(), vec![0, 1, 2]);
        assert_eq!(parallel.add, serial.add);
        assert_eq!(parallel.remove, serial.remove);
    }
}
