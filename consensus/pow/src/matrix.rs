...
#[inline(always)]
fn nibbles_from_hash(hash: &Hash) -> [u8; 64] {
    let mut out = [0u8; 64];
    // SAFETY: indices are in-bounds; loop is fixed size.
...
impl Matrix {
    ...
    fn convert_to_float(&self) -> [[f64; 64]; 64] {
        let mut out = [[0.0f64; 64]; 64];
        for i in 0..64 {
            for j in 0..64 {
                out[i][j] = self.0[i][j] as f64;
            }
        }
        out
    }
    ...
    /// - Single matrix operation for computational complexity
    /// - Vulnerability mitigation against 0x51 attack
    /// Optimized with merged hashing to reduce Hasher creation overhead
    #[inline(always)]
    pub fn swift_heavy_hash(&self, hash: Hash, nonce: u64) -> Hash {
        ...
        hasher.update(&matrix_result.as_bytes());
    ...
    /// Optimized with pre-computed seed_bytes to reduce modulo and shift operations
    #[inline(always)]
    pub fn matrix_vector_multiply_with_seed(&self, hash: Hash, seed: u64) -> Hash {
        let vec = nibbles_from_hash(&hash);
        let seed_bytes = seed.to_le_bytes();

        let mut product: [u8; 32] = [0; 32];

        // SAFETY: we only use in-bounds fixed indices
        let m: &[[u16; 64]; 64] = &self.0;
        ...
        for (i, p) in product.iter_mut().enumerate() {
            *p ^= hash.as_bytes()[i] ^ seed_bytes[i & 7];
        }
...
mod tests {
    ...
    #[test]
    fn test_swift_heavy_hash() {
        // Test the simplified SwiftHeavy algorithm (no scratchpad)
        ...
        assert_ne!(result, different_result2);
    }
    ...
    #[test]
    fn test_0x51_attack_vulnerability_fix() {
    ...
    #[test]
    fn test_nonce_impact_on_performance() {
