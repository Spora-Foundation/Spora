...
use crate::tx::{copperoot::error::CopperootError, Transaction, TransactionInput, TransactionOutput};
...
// Note: Encodable implementations are already provided in taproot/sighash.rs
...
impl<Tx: Borrow<Transaction>> SighashCache<Tx> {
    ...
    pub fn copperoot_key_spend_signature_hash<TxOut: Borrow<TransactionOutput>>(
        ...
        self.copperoot_encode_signing_data_to(&mut data, input_index, prevouts, None, None, sighash_type)?;
