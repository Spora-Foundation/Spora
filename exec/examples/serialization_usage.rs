...
use spora_exec::{
    CellTx, VersionedEnvelope, VersionedSerializable, VmSerializable, ResolvedHeader, ResolvedCell,
    CellOutput, Script, OutPoint, CellInput, VmAbiNegotiator,
};
...
fn store_cell_tx_example() {
    ...
    let restored_envelope: VersionedEnvelope<CellTx> = 
        borsh::from_slice(&storage_bytes).expect("deserialization should succeed");
...
fn vm_abi_example() {
    ...
    let restored_header = ResolvedHeader::from_vm_bytes(&vm_bytes)
        .expect("VM deserialization should succeed");
