...
1. Create an op variant in `tondi_rpc_core::api::ops::RpcApiOps`
   (ie. `SubmitBlock`)
2. Create in `tondi_rpc_core::model::message` a pair of request and response structures
   (ie. `SubmitBlockRequest` and `SubmitBlockResponse`).
3. Implement a constructor for the request.
4. If necessary, implement converters to handle consensus-core <-> rpc-core under `tondi_rpc_core::convert`.
5. Add a pair of new async functions to the `tondi_rpc_core::api::RpcApi` trait, one with detailed parameters
...
6. Implement the function having a `_call` suffix into `tondi_rpc_core::server::service::RpcCoreService`.
...
2. In file `rpc\grpc\proto\messages.proto`, add respectively a request and a response to the payload of `tondidRequest` and `tondidResponse`.
...
9. In `tondi_grpc_server::service::RpcService::message_stream`, requests handler, add an arm and implement
