    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 7 {
            return Err(Error::new_spanned(parsed,
                "usage: build_grpc_server_interface!(server_context, ServerContextType, ConnectionType, TondidRequestType, TondidResponseType, TondidPayloadOps, [GetInfo, ..])".to_string()));
        }
                                ...
                                Box::pin(async move {
                                    let mut response: #tondid_response_type = match request.payload {
                                        Some(Payload::#request_type(ref request)) => {
                                            match tondi_rpc_core::#fallback_request_type::try_from(request) {
                                                Ok(request) => {
