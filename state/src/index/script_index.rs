...
impl ScriptIndex {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let db = DB::open_cf_descriptors(&opts, path, vec![cf_lock, cf_type])
    ...
    fn add_to_index(&self, cf_name: &str, script_hash: &[u8; 32], out_point: &OutPoint) -> Result<()> {
        ...
        let cf = self.db.cf_handle(cf_name)
            .ok_or_else(|| StateError::Database(format!("{} not found", cf_name)))?;
        
        // Get existing OutPoints
        let mut out_points = match self.db.get_cf(&cf, script_hash)
            .map_err(|e| StateError::Database(e.to_string()))? {
        ...
        self.db.put_cf(&cf, script_hash, &value)
            .map_err(|e| StateError::Database(e.to_string()))?;
    ...
    fn remove_from_index(&self, cf_name: &str, script_hash: &[u8; 32], out_point: &OutPoint) -> Result<()> {
        ...
        if out_points.is_empty() {
            ...
            self.db.delete_cf(&cf, script_hash)
                .map_err(|e| StateError::Database(e.to_string()))?;
        } else {
