...
| Spora scheduler witness metadata | V1-complete for in-process path | Compiler metadata emits public Molecule scheduler witnesses; Borsh is explicit migration/regression only. `ActionMetadata`, exec admission, consensus, mining, and wallet paths reject malformed/legacy/conflicting public inputs as covered by the release gate. |
