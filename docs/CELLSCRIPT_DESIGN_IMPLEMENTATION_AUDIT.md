...
| Type checking / linear checks | 68-75% | Stronger value/resource checks, including linear `let` move semantics and explicit branch-return ownership merging; still not full semantic proof |
...
|---|---|---:|
| `resource` | Parsed, typed, lowered into IR, participates in partial linear checks and metadata; local `let` bindings over linear values move the source instead of copying it, and explicit branch returns now merge terminal linear ownership state; one external resource Input preserved into one same-type Output by direct verifier-covered field aliases, restricted single-field `amount: u64` additive merges from multiple same-type Inputs into one Output, and restricted one-Input matched `amount` subtraction splits into sibling Outputs are now classified as `resource-conservation:<T>` `checked-runtime` and surfaced as checked `resource-conservation-proof` transaction input components | 79-84% |
