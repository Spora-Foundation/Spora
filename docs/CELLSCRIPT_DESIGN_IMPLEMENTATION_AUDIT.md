...
- The machine-block layer now stores the derived `MachineCfg` inside the layout plan, derives edges for fallthrough, jump, and conditional branch flow, validates that machine blocks cover every executable text op exactly once, owns a validated `MachineLayoutOrder` permutation over CFG blocks with planned `MachinePlacedBlock` byte offsets, rejects control-flow targets that do not start text blocks, and reports edge/unreachable-block/coverage/layout-order counts as backend layout metrics. VM syscalls (`ecall`) are modeled as fallthrough instructions in this CFG because CellScript checks their return codes instead of treating them as terminal exits.
- Bundled examples are covered by size, assembly-shape, and backend-shape budgets through `analyze_backend_shape`, including maximum machine-block size and unreachable-block count. `BackendShapeMetrics` is serializable, and the examples test suite can emit a JSON backend-shape report with `CELLSCRIPT_BACKEND_SHAPE_REPORT=/path/to/report.json`. The release gate checks that the layout plan, machine-block/CFG layer, public backend-shape analyzer, serializable report path, branch-relaxation metric test, explicit machine-block/CFG test, invalid CFG target test, and shape-budget test remain present.
