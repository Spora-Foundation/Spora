...
- `366` direct `#[test]` declarations are present in source/test files.
- A fresh default-feature `cargo test -p cellscript` run executed `374` tests: `306` library tests, `61` CLI integration tests, `7` examples integration tests, and `0` doctests. All passed.
...
| IR and metadata | 76-86% | Real, includes verifier obligations and schema v26 target profile metadata; still not full protocol semantics |
...
| DAG scheduler integration | 80-86% | Metadata and per-action scheduler witness bytes exist; `read_ref`, `&mut shared`, composed shared-return touches, and mutable Cell `mutate-input` / `mutate-output` access records are visible; mutable shared-state and mutable authority cell-state transition obligations are explicit; schema v26 keeps scheduler witnesses limited to Input/CellDep/Output cell-state accesses; `ActionMetadata::scheduler_witness_bytes()` exposes launch Molecule compiled witness bytes, and `spora-exec` CellTx can attach/discover/admit public Molecule CellScript scheduler witnesses, while legacy `0xCE11` Borsh magic/version decoding is crate-internal migration/regression coverage only and is not public admission. Public Molecule admission checks magic/version/count, effect/operation/source, operation/source compatibility, concrete transaction source-index bounds, exact trusted operation/source/index/binding_hash access-set matching, and can produce a trusted access summary from compiled metadata bytes while appending the witness to a concrete transaction. Consensus MPE `BlockAccessSummary` consumes admitted witnesses, has a strict trusted-access-set constructor that rejects missing/mismatched builder or compiled-metadata summaries before merge, and uses shared read/write touch domains for DAG serialization. Mempool validation and template prefiltering now reject malformed CellScript scheduler metadata; mempool entries and template selectors can carry producer-backed trusted summaries into the strict template policy path. Wallet transaction generation can attach a compiled scheduler witness to the final transaction and expose the returned trusted summary on `PendingTransaction`; focused mining coverage proves producer-returned summaries survive sidecar insertion into selector exposure, and focused consensus coverage proves selector-provided builder summaries are consumed/rejected by strict template prefiltering. RPC trusted-summary submission/authentication and broader adversarial/property coverage remain post-v1 hardening |
| CLI local workflow | 74-84% | Usable local developer loop with pre-artifact CLI/manifest metadata policy gates and explicit `spora` / `ckb` / `portable-cell` target-profile selection; `ckb` can produce artifacts for the pure supported subset, while non-portable/stateful features fail closed through target-profile policy; `cellc check --target-profile ckb|portable-cell` runs portability classification and reports concrete blockers |
...
## Backend Architecture And Control Gates

The backend long-branch incident has moved from one-off repair into ongoing compiler architecture work:

- The assembler safety net is now permanent for out-of-range conditional branches.
- Codegen has started structural cleanup through shared epilogues, cold fail handlers, and shared fixed-byte/size guard helpers.
- The ELF path now builds a `MachineLayoutPlan` before final emission, giving the backend an explicit layout/CFG/metric boundary instead of relying on direct textual emission alone.
- The layout plan now derives explicit `MachineBlock` records with `MachineTerminator` classification, so conditional branch, jump, return, and fallthrough regions are inspectable before final encoding.
- The machine-block layer now stores the derived `MachineCfg` inside the layout plan, derives edges for fallthrough, jump, conditional branch flow, and local `call` targets, validates that machine blocks cover every executable text op exactly once, owns a validated `MachineLayoutOrder` permutation over CFG blocks with planned `MachinePlacedBlock` byte offsets, rejects control-flow targets that do not start text blocks, and reports edge/call-edge/unreachable-block/coverage/layout-order counts as backend layout metrics. VM syscalls (`ecall`) are modeled as fallthrough instructions in this CFG because CellScript checks their return codes instead of treating them as terminal exits.
- Bundled examples are covered by size, assembly-shape, and backend-shape budgets through `analyze_backend_shape`, including maximum machine-block size, call edge count, and entry-root unreachable-block count. `BackendShapeMetrics` is serializable, and the examples test suite can emit a JSON backend-shape report with `CELLSCRIPT_BACKEND_SHAPE_REPORT=/path/to/report.json`; the release gate now sets this variable by default and writes `target/cellscript-backend-shape/backend-shape-report-<mode>.json`. The release gate checks that the layout plan, machine-block/CFG layer, public backend-shape analyzer, serializable report path, branch-relaxation metric test, explicit machine-block/CFG test, call-edge test, entry-root reachability test, invalid CFG target test, and shape-budget test remain present.

This is not yet a full low-level machine optimizer. Block layout, tail merging, cold-path sinking, helper placement, and graph-level optimization passes remain open Phase C work. Phase D has started because code size, helper counts, branch relaxation, and bundled example shape are now CI/release-gate concerns rather than manual observations.

## V1 Close Interpretation

For this audit, "v1 closed" means the executable-core boundary is closed. It
does not mean the full design proposal, the generalized protocol language, or a
release-tagged external product surface is complete.

No remaining item is required to close the current v1 core-language convergence
gate. The supported core features have stable checked/runtime/fail-closed
classification, and the residual core-adjacent gaps are visible through stable
blocker classes.

The remaining work before a release tag or broad external compatibility
commitment is:

- rerun the full release gate on the final worktree after all release changes land
- either implement or explicitly defer generalized resource conservation beyond the restricted checked subsets
- keep unsupported transfer, claim, settle, and finalization output relations policy-visible until executable
- keep AMM/Pool economic invariants, generalized Pool admission, and first-class Pool/launch semantics out of v1 core unless the gate is reopened
- keep external RPC trusted-summary submission out of scope unless RPC-side builders are admitted with an authenticated trust policy
- document SDK, LSP, package, registry, Wasm, and developer-workflow limits as product/tooling debt rather than compiler-core blockers
...
- `transfer`, `claim`, and `settle` are still not complete executable protocol semantics; `destroy` now has a restricted executable transaction Output TypeHash absence scan, but full burn/conservation policy is still incomplete.
...
| Schema evolution/versioning | `#[type_id("...")]` stable type identity is parsed for `resource` / `shared` / `receipt` / `struct`, duplicate IDs are rejected across the visible module scope including imported types, IR carries the string, and metadata schema v26 emits both `types[].type_id` and a BLAKE3 hash. Under the `ckb` profile, persistent Cell types also emit `types[].ckb_type_id` with the CKB built-in TYPE_ID script contract, and direct create outputs emit `create_set[].ckb_type_id` output plans; wallet builders can explicitly install TYPE_ID scripts by final output index, native/WASM generator settings can consume CellScript action metadata profile-aware, and native/WASM generator settings can carry explicit CKB deps/header deps, while higher-level transaction-builder invocation and schema migration rules remain incomplete | 50-58% |
...
| `burn` / `destroy` | `destroy` recognized and now checks transaction Output TypeHash absence for named cell-backed operands; generalized burn policy and conservation accounting remain incomplete | 24-30% |
...
- fixed-byte output preservation compares `Address`, `Hash`, and fixed `[u8; N]` fields for constants, all-zero constants through the shared `__cellscript_memzero_fixed` helper, schema-backed aliases, stack-backed `[u8; N<=8]` parameters, pointer+length `Address` / `Hash` parameters, trusted schema-parameter TypeHash ABI bytes, and created Output TypeHash fields
...
- verifier-coverable fixed-byte equality/inequality now lowers to shared helper calls for pointer-backed or all-zero comparisons where possible, falling back to byte-wise assembly for matching-width constants, schema-backed fields, small stack values, and fixed-byte pointer parameters
- scalar `let` annotations are preserved in IR, allowing source-order create checks to verify narrow lifecycle state fields such as `u8`; fieldless enum storage width and pure literal-return summaries now let fixed-width examples such as `timelock.cell` verify enum fields plus `Hash::zero()`-style output fields without treating payload enums, `String`, `Vec`, or dynamic indexes as covered
...
- effect classes
- per-action launch Molecule scheduler witness bytes with operation/source/index/binding-hash access records in metadata (`scheduler_witness_hex` + `scheduler_witness_abi = "molecule"`), filtered to scheduler-visible Input/CellDep/Output accesses
