...
- `cellscript/src/` contains `41,317` lines of Rust across `26` source files.
- `cellscript/src/` plus `cellscript/tests/` contains `45,697` lines of Rust across `28` files.
...
|---|---|---:|
| `resource` | Parsed, typed, lowered into IR, participates in partial linear checks and metadata; local `let` bindings over linear values move the source instead of copying it, aggregate types containing linear values are linear, wildcard bindings cannot discard linear values, field/index projection cannot move linear elements out of aggregate values before partial-move semantics exist, explicit branch returns, tail-if return expressions, `if` expression branches, `match` expression arms, block-expression scopes, and block expressions whose tail statement is an `if` with `else` now propagate parent-visible linear ownership state while rejecting unhandled block-local linear values and inconsistent branch/match moves; plain `consume` emits checked `consume-input-data` transaction input components for the concrete Input data-load path, direct `create` output field/lock verifier gaps are now exposed as transaction blocker classes, one external resource Input preserved into one same-type Output by direct verifier-covered field aliases, restricted single-field `amount: u64` additive merges from multiple same-type Inputs into one Output, and restricted one-Input matched `amount` subtraction splits into sibling Outputs are now classified as `resource-conservation:<T>` `checked-runtime` and surfaced as checked `resource-conservation-proof` transaction input components | 87-92% |
