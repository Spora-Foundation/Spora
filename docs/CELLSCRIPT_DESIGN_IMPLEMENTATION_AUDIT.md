...
- `cellscript/src/` contains `41,137` lines of Rust across `26` source files.
- `cellscript/src/` plus `cellscript/tests/` contains `45,506` lines of Rust across `28` files.
...
| Scheduler metadata has no fail-closed signal | Improved. Metadata exposes `fail_closed_runtime_features` separately from `symbolic_runtime_features`, including dedicated Cell-backed collection markers plus `linear-collection-ownership-gap` runtime input blockers for action-visible `Vec<CellType>` paths that still require a real linear collection model. |
