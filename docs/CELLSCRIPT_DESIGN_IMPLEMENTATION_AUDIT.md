...
- `cellscript/src/` contains `39,375` lines of Rust across `26` source files.
- `cellscript/src/` plus `cellscript/tests/` contains `43,744` lines of Rust across `28` files.
- `342` `#[test]` declarations are present in source/test files.
- A fresh default-feature `cargo test -p cellscript` run executed `329` tests: `273` library tests, `49` CLI integration tests, `7` examples integration tests, and `0` doctests. All passed.
...
| Type checking | Useful and stricter on schema field identity, callable parameter identity, local binding identity, named-root and read-only-reference assignment identity, callable argument count/type checks, returns, unreachable statements, assertions, empty arrays, `Unit`, local `Vec` item propagation, aggregate-contained linear values, and scoped linear-state propagation; still not full semantic proof | 74-81% |
