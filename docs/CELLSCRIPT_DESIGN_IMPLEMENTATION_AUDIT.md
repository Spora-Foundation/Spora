...
- Bundled examples are covered by size, assembly-shape, and backend-shape budgets through `analyze_backend_shape`, including maximum machine-block size, call edge count, and unreachable-block count. `BackendShapeMetrics` is serializable, and the examples test suite can emit a JSON backend-shape report with `CELLSCRIPT_BACKEND_SHAPE_REPORT=/path/to/report.json`; the release gate now sets this variable by default and writes `target/cellscript-backend-shape/backend-shape-report-<mode>.json`. The release gate checks that the layout plan, machine-block/CFG layer, public backend-shape analyzer, serializable report path, branch-relaxation metric test, explicit machine-block/CFG test, invalid CFG target test, and shape-budget test remain present.
