...
|---|---|---:|
| `resource` | Parsed, typed, lowered into IR, participates in partial linear checks and metadata; one external resource Input preserved into one same-type Output by direct verifier-covered field aliases, restricted single-field `amount: u64` additive merges from multiple same-type Inputs into one Output, and restricted one-Input matched `amount` subtraction splits into sibling Outputs are now classified as `resource-conservation:<T>` `checked-runtime` and surfaced as checked `resource-conservation-proof` transaction input components | 77-82% |
