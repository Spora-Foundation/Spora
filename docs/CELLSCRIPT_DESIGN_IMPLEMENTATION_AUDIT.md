...
| Resource conservation | Partial restricted subset | Simple fixed-scalar output equality, schema-backed fixed-byte preservation, and u64 arithmetic checks exist; direct one-external-input/one-created-output same-type field aliases and restricted single-field `amount: u64` additive merges from every consumed same-type Input into one Output are classified as `resource-conservation:<T>` `checked-runtime`, while subtractive net-amount, multi-output, extra-field, and broader cross-cell accounting stay runtime-required. |
