...
| Protocol semantic lowering | IR records operation-tagged `consume_set`, operation-tagged `read_refs`, and operation-tagged `create_set`; generated metadata includes runtime access summaries and scheduler witness hex with operation/source/index/binding-hash access records. Generated assembly now rejects unsupported symbolic runtime operations instead of leaving them as successful silent paths. | Access summaries and fail-closed paths are real, but not yet equivalent to a verified state-transition proof. | Connect access summaries to executable verifier code and scheduler admission checks. |
...
| Access summary metadata | `metadata_schema_version`, `compiler_version`, `artifact_hash_blake3`, `artifact_size_bytes`, `source_hash_blake3`, `source_content_hash_blake3`, `source_units[]`, `types[].capabilities`, `types[].claim_output`, `params`, `functions[]`, operation-tagged `consume_set`, operation-tagged `read_refs`, operation-tagged `create_set`, `ckb_runtime_accesses`, `ckb_runtime_features`, `runtime.vm_abi`, `standalone_runner_compatible`, `symbolic_runtime_features`, `fail_closed_runtime_features`, `verifier_obligations`, and `elf_compatible` are emitted as JSON. | CI/audit tooling can distinguish pure helpers, type capability authority, receipt-to-output type promises, checked-static resource-operation gates, runtime-required transaction invariants, source provenance, path-bound source identity, path-independent source content identity, CKB-runtime ELF, standalone runner ELF, required VM object ABI, symbolic requirements, explicit fail-closed runtime paths, lowering obligations, whether an Input access came from `consume`, `transfer`, `destroy`, `claim`, or `settle`, and whether a cell-backed Output access came from `create`, `transfer`, or `claim`. Scheduler witness version 1 also carries operation/source/index/binding-hash records so a DAG scheduler can consume compact Borsh metadata without depending solely on JSON. Compile results now self-validate schema/toolchain compatibility plus artifact/source/metadata binding, but scheduler/runtime policy still has to consume it. | Add CI policies and runtime checks that reject inconsistent metadata/artifacts. |
| DAG integration | Scheduler metadata and Borsh witness bytes are emitted; witness version 1 includes effect class, shared-touch hashes, estimated cycles, and operation-tagged access records. | This is a real compiler output path, but not a completed DAG scheduler integration. | Feed metadata into DAG access/conflict logic and add adversarial scheduling tests. |
| CLI ecosystem | `build`, `check`, `doc`, `fmt`, `init`, `add`, `remove`, `clean`, `info`, `metadata`, `verify-artifact`, package compilation, compile-test discovery, and feature-gated `run` have real paths. `build` and `check` now have CLI and manifest `[policy]` metadata gates for production, fail-closed, symbolic runtime, CKB runtime, and runtime-required verifier obligations; `build` applies the gate before writing artifacts and `build --json` emits a machine-readable artifact/metadata/hash summary with fail-closed/runtime-required obligation counts; `check --all-targets` validates both asm and ELF lowering without writing artifacts, and `check --json` emits a machine-readable checked-target/policy summary with obligation counts. `init --json`, `add --json`, `remove --json`, `clean --json`, and `info --json` cover local package lifecycle summaries for CI/IDE callers; `add/remove --dev/--build` now target the correct manifest sections and `add --git/--path` writes detailed dependency sources. `verify-artifact` validates artifact/metadata binding for CI without recompilation, `--verify-sources` checks source-unit hashes against files on disk, `--expect-*hash` pins artifact/source/source-content BLAKE3 values, `--production` / `--deny-*` applies the same metadata policy gates to stored artifacts, including runtime-obligation rejection, and `--json` emits a machine-readable summary with obligation counts. `doc` emits API docs plus lowering audit reports and verifier obligations. Registry/package network commands remain fail-closed. | Core local developer workflow is usable; ecosystem distribution is not complete. | Finish registry install/publish/update and expand CI policy presets. |
...
- declared `receipt Name -> Output` claim outputs are type-checked as resource/shared cells and represented as operation-tagged Output accesses
...
- `touches_shared`
- operation-tagged `consume_set`, operation-tagged `read_refs`, operation-tagged `create_set`
- per-action scheduler witness Borsh hex with operation-tagged access records
