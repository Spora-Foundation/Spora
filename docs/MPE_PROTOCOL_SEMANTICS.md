...
- 日期: 2026-04-15
- 状态: Normative（规范性）
- 相关文档: [MPE_VIRTUAL_PROCESSOR_PARALLELIZATION_DESIGN.md](./MPE_VIRTUAL_PROCESSOR_PARALLELIZATION_DESIGN.md)

---

## 0. 本文档的定位

本文档是 MPE Virtual Processor 并行化重构的**前置规范**。在动代码前，必须先明确 5 个协议语义问题。后续所有实现（包括串行 reference runner 和并行 runner）**不允许偏离**本文档定义的规则。
...
**场景**：analyze 阶段基于某个 snapshot 生成了一个 block 的 effect，但在提交（commit）到 canonical state 时，发现该 effect 的前提已失效（例如：该 effect 消费的 input outpoint 或读取的 cell_dep outpoint 已被先前提交的 effect 消费）。
...
- commit 阶段的前提检查必须同时覆盖 effect 的消费集（inputs）和读取集（cell_deps）。只检查消费集是不完整的。
