...
| Post-v1 templates / `#[type_id]` / schema evolution | User-defined generics are no longer a v1 executable-core target: generic type definitions and user-defined instantiations fail closed, while `Vec<T>` remains a controlled builtin collection notation. Parametric authoring is delegated to a post-v1 package/codegen/template layer that must generate concrete `.cell` schemas. `#[type_id("...")]` is now a real item-level attribute for `resource` / `shared` / `receipt` / `struct`: the parser accepts it, the type checker rejects duplicate stable IDs across the visible module scope including imported types, IR preserves it, and metadata schema v20 emits `types[].type_id` plus `types[].type_id_hash_blake3`. Executable CKB type-id lineage verification and full schema migration/versioning rules remain incomplete | ~35-45% for v1 scope |
