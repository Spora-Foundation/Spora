...
| `transfer` / `destroy` | Parsed/lowered and capability-gated; `transfer` can preserve verifier-coverable fields and now carries the destination `Address` into the transfer-created output pattern so verifier-coverable Output LockHash rebinding plus destination address binding are `checked-runtime`; when that operation-tagged output relation is fully verifier-covered, the `transfer` expression no longer emits the symbolic fail-closed path or `transfer-expression` fail-closed metadata; named cell-backed `destroy` now emits an executable `GroupOutput` TypeHash absence scan and marks destroy output absence/group-boundary inputs as `checked-runtime`; generalized transfer semantics, broader burn/conservation policy, and full semantics remain incomplete | ~38-46% |
