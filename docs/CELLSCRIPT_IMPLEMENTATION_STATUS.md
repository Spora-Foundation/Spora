...
| `consume` / `create` | Real syntax/type/IR/codegen path, with restricted fixed-scalar and schema-backed fixed-byte verifier support; direct consumed-field aliases into a same-type created resource and restricted multi-input `amount: u64` additive merges now emit `resource-conservation:<T>` `checked-runtime`, while net-amount, multi-output, or richer cross-cell conservation remains `runtime-required` | ~70-80% |
