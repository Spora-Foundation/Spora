...
- Rust source size: `39,205` lines across `26` files under `cellscript/src/`; `43,574` lines across `28` Rust files when `cellscript/tests/` integration tests are included.
- Test inventory: `339` `#[test]` declarations are present in source/test files; the default-feature active suite run below executed `326` tests.
...
| Type checker | Production-hardening | `cellscript/src/types/` | Real type/effect/call-argument/resource-operation checks exist; schema field names and callable parameter names must now be stable and unique before IR/metadata lowering; this is still not a complete semantic proof system |
