...
- restricted `Token { amount: u64 }`-style resource merges where every consumed same-type Input contributes exactly one `amount` leaf to one created Output's left-associative `u64 Add` expression are classified as `resource-conservation:<T>` `checked-runtime`; subtractive fees, duplicate leaves, missing consumed inputs, extra fields, multi-output splits, and richer accounting stay `runtime-required`
