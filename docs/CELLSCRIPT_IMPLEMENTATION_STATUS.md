...
- Rust source size: `39,271` lines across `26` files under `cellscript/src/`; `43,640` lines across `28` Rust files when `cellscript/tests/` integration tests are included.
- Test inventory: `340` `#[test]` declarations are present in source/test files; the default-feature active suite run below executed `327` tests.
...
| Type checker | Production-hardening | `cellscript/src/types/` | Real type/effect/call-argument/resource-operation checks exist; schema field names, callable parameter names, and local binding names must now be stable and non-overwriting before IR/metadata lowering; this is still not a complete semantic proof system |
