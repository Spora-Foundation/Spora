...
- Rust source size: `39,144` lines across `26` files under `cellscript/src/`; `43,513` lines across `28` Rust files when `cellscript/tests/` integration tests are included.
- Test inventory: `338` `#[test]` declarations are present in source/test files; the default-feature active suite run below executed `325` tests.
...
| Type checker | Production-hardening | `cellscript/src/types/` | Real type/effect/call-argument/resource-operation checks exist; callable parameter names must now be stable and unique across `action` / `fn` / `lock`; this is still not a complete semantic proof system |
