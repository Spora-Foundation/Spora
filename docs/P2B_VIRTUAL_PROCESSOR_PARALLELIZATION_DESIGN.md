...
- `P2b`：virtual processor 在处理一个 mergeset 的多个 blue blocks 时，能不能并行推进 Cell 状态
...
- `P2b` 是“一个 DAG 视角下有很多 blue blocks 要一起算进虚拟状态，能不能同时算”
...
`P2b` 是不是一个值得做的吞吐优化，以及它在不改变共识语义的前提下应该怎么做。
...
| `P2b` | mergeset 内多个 blue blocks 的 virtual-state 推进 | 否 | 是 | 只能先做前置重构 |
...
- `P2b` 值得做，但它是吞吐优化工程，不是短平快补丁
- `P2b` 现在还不应该直接开工做“跨 blue block 并行执行”
- 不需要修改 GhostDAG 模型
- 需要先重构 `virtual_processor`，把“逐块重放并直接改共享状态”改成“生成局部 effect，再按确定性顺序合并”

一句话总结：

P2b 的前提不是更多线程，而是先把 virtual-state 执行语义变成可组合、可验证、可顺序合并的 effect 模型。
...
- `P2b` 这种并行，CKB 天然没有同等问题空间，因为它没有 mergeset blue blocks 这层执行语义

所以：

- `P2b` 不是“落后于 CKB 的缺功能”
- `P2b` 是 GhostDAG 架构自己带来的额外复杂度

截至 2026-04-12，我对照了 CKB 官方主线实现，至少在主验证路径上可以看到：

- 块内交易验证仍以串行遍历为主
- 脚本组验证在官方实现里也不是“像 P2b 那样”的共识层跨任务并行
...
P2b 不能等同于“把 scheduler 直接接进 virtual_processor”。
...
所以 `P2b` 的核心工作量，不在并发 API，而在执行模型本身。
...
P2b 是一个需要前置重构的 P2 项，不是一个可以立刻落地的 P1 优化项。
...
GhostDAG 当前已经提供了 P2b 所需的核心共识语义：
...
- 不需要改 RPC 对外协议才能开始做 `P2b`

`P2b` 首先是 internal execution refactor，不是协议格式升级。
...
下面这些问题，如果答案还没定，就不应该开始写 `P2b` 的并行代码：
...
P2b 的问题不是 DAG 模型不够，而是执行层没被正确分层。
...
3. cross-blue-block duplicate tx / double spend 场景有明确规范测试
...
P2b 值得做，但先重构执行语义；GhostDAG 不需要改。
...
- 说明 CKB 没有与 Spora `P2b` 完全同型的问题空间
