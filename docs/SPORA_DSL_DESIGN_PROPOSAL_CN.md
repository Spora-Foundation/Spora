...
| 测试套件 | ✅ 默认特性 `cargo test -p cellscript` 当前 350 项全部通过 | `cellscript/src/`, `cellscript/tests/` |
...
当前实现说明：`#[type_id("...")]` 是 type definition 级属性，可用于 `resource` / `shared` / `receipt` / `struct`。编译器会解析它、拒绝同模块重复值、在 IR 中保留，并在 metadata schema v21 中输出 `types[].type_id` 和 `types[].type_id_hash_blake3`。这还不是完整的 CKB type_id lineage verifier；验证 Cell 的 OutPoint 链追溯到创世交易仍属于后续可执行 verifier / transaction-builder 语义。
