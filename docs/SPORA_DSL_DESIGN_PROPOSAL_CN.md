...
`scheduler_witness_molecule_hex` 或 `scheduler_witness_borsh_hex`。公开默认读取路径会拒绝 legacy Borsh 字段和冲突的 Molecule alias；旧 `scheduler_witness_molecule_hex` 只在 launch 字段缺失时作为迁移读取路径保留，legacy Borsh 只保留在 crate-internal 迁移/回归 decode 路径。`spora-exec` 的 `CellTx`
...
放置/发现/解码/admission helper，公开 admission 只接受 Molecule，legacy Borsh 只保留在 crate-internal 迁移/回归 decode 路径；共识 MPE `BlockAccessSummary` 已消费 transaction-admitted witness。
...
Slice 65 更新：带显式 20-byte signer 字段且包含源级 `assert_invariant` / DAA 谓词的 guarded claim 现在有独立 blocker surface。若源谓词没有对应 verifier-covered guard，编译器保留 `claim-conditions:<Receipt>` 顶层 `runtime-required` 和 `source-predicate=runtime-required`，同时额外生成 `transaction_runtime_input_requirements[]` 中的 `claim-source-predicate` 组件，source 为 `Transaction`、field 为 `source-predicate`、ABI 为 `claim-source-predicate-cfg`，blocker class 为 `claim-source-predicate-gap`。若源谓词已被 `daa-cliff-reached` / `source-invariant` 等 checked guard 覆盖，则不再生成该 blocker，并可把 signer-backed claim conditions 整体升级为 `checked-runtime`。这样 CLI/CI 不再只能从 claim 条件长文本里推断未覆盖的源谓词，而可以直接按 blocker class 拒绝或统计该类差距。
