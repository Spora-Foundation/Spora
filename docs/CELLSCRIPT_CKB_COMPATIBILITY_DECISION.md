...
- The pure baseline is a strict CKB artifact. Bundled examples use CKB smoke artifacts for the generic on-chain locked-cell spend: the script copies the fixed seven-file example set, appends a no-argument `main() -> u64 { 0 }` smoke entry, and sets `CELLSCRIPT_CKB_ACCEPTANCE_SMOKE_ALLOW_UNPORTABLE_EXAMPLES=1` so packaging, CKB-VM loading, code-cell dependency resolution, and lock-script invocation can be tested before all business-action transaction harnesses are admitted. The compiler only honors that bypass for the exact env value `1`, the `ckb` profile, and a source module with a no-argument `main() -> u64`.
- The passing claim now includes strict CKB compile/sidecar verification for `token.cell`, baseline execution, smoke execution for the bundled examples, action-specific CKB on-chain execution for `token.cell` `mint`, `transfer_token`, `burn`, and `merge`, action-specific CKB on-chain execution for the fixed-width `nft.cell` subset `transfer` and `burn`, and action-specific CKB on-chain execution for the fixed-width `timelock.cell` subset `extend_lock`. These action harnesses construct real CKB witnesses, input cell data, output cell data, type/dependency wiring, valid transactions, and malformed transactions that must be rejected by script logic.
...
- The acceptance matrix now hard-runs the pure baseline and all seven bundled examples in this exact order: `amm_pool.cell`, `launch.cell`, `multisig.cell`, `nft.cell`, `timelock.cell`, `token.cell`, `vesting.cell`. Latest passing full CKB report: `target/ckb-cellscript-acceptance/20260421-115555-16636/ckb-cellscript-acceptance-report.json`. It records `bundled_examples_exact_order`, `strict_original_ckb_compile_policy_fail_closed`, `strict_original_ckb_compile_unexpected_failures = []`, per-example `acceptance_smoke_policy_bypass`, `onchain.all_artifacts_deployed_and_spent = true`, per-artifact `valid_spend_dry_run`, malformed missing-dep rejection, and committed spend recipients for every example.
- Follow-up CKB strict-admission fix: `token.cell` now compiles and verifies as a strict original CKB artifact without the acceptance smoke bypass. The compiler recognizes verifier-covered resource merge conservation for `amount` sums plus fixed identity fields when the source has an explicit equality guard such as `a.symbol == b.symbol`; unsound identity-field merges without that guard remain `runtime-required` and are still rejected by CKB target-profile policy. The acceptance report now records `bundled_examples_strict_admitted` separately from the smoke artifacts used for generic on-chain spend testing.
- Follow-up CKB on-chain action harness fix: latest passing full CKB report `target/ckb-cellscript-acceptance/20260421-174830-52080/ckb-cellscript-acceptance-report.json` records `onchain.token_actions_exercised = ["mint", "transfer_token", "burn", "merge"]` and `onchain.all_token_actions_exercised = true`. This run exposed and fixed three executable lowering gaps: mutable schema parameters are now rebound to loaded Input cell data before field access, `destroy` operands now participate in consumed schema pointer binding, and destroy absence scans compare Output TypeHash values against the consumed Input's real CKB TypeHash while clearing syscall loop return codes before success return.
- Follow-up NFT action harness fix: latest passing full CKB report `target/ckb-cellscript-acceptance/20260421-180703-87899/ckb-cellscript-acceptance-report.json` records `onchain.nft_actions_exercised = ["transfer", "burn"]` and `onchain.all_nft_actions_exercised = true` in addition to the token action checks. This run exposed and fixed the direct-assignment mutable transition gap: `field = operand` is now represented as a `Set` transition in IR/metadata, fixed-width output fields such as `NFT.owner: Address` are byte-verified against the action parameter under the CKB profile, and the harness uses CKB-real occupied-capacity-safe NFT cells instead of undersized smoke cells.
- Follow-up TimeLock action harness: latest passing full CKB report `target/ckb-cellscript-acceptance/20260421-181811-10366/ckb-cellscript-acceptance-report.json` records token, NFT, and `onchain.timelock_actions_exercised = ["extend_lock"]` with all action families marked exercised. The TimeLock harness verifies fixed-byte owner checks plus scalar `unlock_height = old + additional_period` replacement-output transition on a real CKB transaction, and rejects malformed replacement output height by script logic.
- Follow-up fixed-width TimeLock verifier closure: the CellScript backend now treats fieldless enums as fixed 1-byte schema fields, summarizes pure functions that unconditionally return literal constants, and uses a shared all-zero fixed-byte helper to avoid re-inflating generated assembly. As a result, `timelock.cell` direct creates for `create_absolute_lock`, `create_relative_lock`, `request_release`, `execute_release`, and `execute_emergency_release` are classified as `checked-runtime` for `create-output-fields`; `LockedAsset`, `EmergencyRelease`, and `batch_create_locks` correctly remain runtime-required because they depend on payload enum data, `String` / `Vec` data, or dynamic collection indexes. Latest compile-only CKB report after this closure: `target/ckb-cellscript-acceptance/20260421-202504-48277/ckb-cellscript-acceptance-report.json`.
- Follow-up dual-profile rerun after CellScript was split to an independent repository/submodule: latest passing Spora full report `target/devnet-acceptance/20260421-232523-19363/acceptance-report.json` records `smoke`, `external_boot`, `propagation`, and `cellscript` as passed at Spora commit `b8c35804`; latest passing full CKB report `target/ckb-cellscript-acceptance/20260421-231905-5630/ckb-cellscript-acceptance-report.json` records `onchain.all_artifacts_deployed_and_spent = true`, token actions `["mint", "transfer_token", "burn", "merge"]`, NFT actions `["transfer", "burn"]`, and timelock actions `["extend_lock"]`. This run exposed and fixed one CKB VM-visible return-code bug: void actions now clear `a0` before the success epilogue so a previous helper/comparison result cannot leak out as `ValidationFailure error code 1`; the fix is in CellScript commit `cb0f697` and the Spora submodule pointer update is in `b8c35804`.
