...
- The passing claim now includes strict CKB compile/sidecar verification for `token.cell`, plus baseline execution and smoke execution for the bundled examples. Strict business-action execution still requires action-specific witnesses, input cell data, output cell data, type/dependency wiring, and executable verifier lowering. Original strict CKB compilation of the other business actions is recorded in the report and still fails closed for the known post-v1 stateful/runtime gaps.
