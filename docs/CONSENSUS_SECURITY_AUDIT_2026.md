...
- Spora 的 Cell / CellTx 概念设计总体成立，主路径迁移已经完成，剩余问题主要集中在资源模型与内部抽象收口。
- “脚本验证完全缺失”“CellValidator 未接入共识”这两条结论已不成立。
- 仍然成立并值得优先处理的，是：
  - Cell mass 仍是简化实现
  - `CellMeta` / metadata / wrapper 的分层边界仍可继续收敛
  - 历史审计与说明文档有一部分口径已落后于代码现状
...
| HIGH | 2 | ⚠️ 持续跟踪 | 资源模型、状态语义与内部分层 |
| MEDIUM | 4 | 📋 按计划处理 | 实现债、分层问题、工程质量 |
...
`脚本验证已接入 block body validation 主路径，并已完成 Cell-native 收口。`

### 2. “CellValidator 未被调用”

该结论也已过时。

`CellValidator` 当前确实被 body processor 调用；问题不在“未接入”，而在资源模型、内部抽象边界和工程收口仍可继续优化。
...
- [ ] 不再依赖旧 `Transaction` 语义下遗留的 compute/storage mass 含义
- [ ] 考虑引入 post-VM 实际 cycles 反馈到 mass 计算

### HIGH-2. `CellMeta` 分层仍有重复定义与边界不清
...
- 代码主路径已经不再依赖旧脚本系统或 txscript
- 已修复历史上“resolved metadata 悄悄改变签名语义”的兼容风险
...
**当前更准确的结论**:
> "Spora 的 Cell 共识设计已经形成可工作的主路径。当前高优先级工作不再是删除 legacy bridge，而是继续完善 Cell-native 资源模型、压缩内部抽象边界，并保持审计文档与实现状态一致。"
...
| 删除准备度 | A- | ↑ | 主路径 bridge 已删除，剩余为文档与内部抽象收尾 |
...
3. [ ] 移除旧 `Transaction` 语义残留的 mass 兼容层
...
### 阶段三：仓库收尾 (P2)
7. [ ] 对齐历史审计文档与 README
8. [ ] 评估 `database/src/registry.rs` 中历史枚举名的处理策略
9. [x] integration crate 编译验证已补齐（`cargo check -p spora-testing-integration --tests`）
...
| 状态 | 含义 | 示例 |
|---|---|---|
| `Open` | 仍成立，尚未修复 | HIGH-1, HIGH-2, MEDIUM-* |
