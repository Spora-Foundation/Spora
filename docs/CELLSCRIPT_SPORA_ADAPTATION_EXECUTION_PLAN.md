...
9. Wallet 已新增 live typed-cell scheduler witness helper，并接入 `Generator` final tx 路径：当 `typed_cell_scheduler_plan` 存在时，Generator 优先用显式 typed output 配置或 resolved Input/CellDep sidecar 生成 live Molecule witness，不再把 compile-time shape witness 当作最终调度凭证。
...
下一阶段重点转为把具体 action-builder tx skeleton 的 typed-cell cell 构造补齐：自动提供 Input/CellDep resolved sidecar 与 typed output 配置，并把 live witness 覆盖从 invoice financing 推广到更多 typed-cell examples；production evidence 已能追踪 scheduler plan / live witness 覆盖缺口。
...
3. Generator 已在 final tx 路径优先使用 live typed-cell scheduler witness，并支持 typed output type script/data 配置；下一步是 invoice financing 等具体 tx skeleton 自动提供 resolved sidecar 与 typed output 配置。
