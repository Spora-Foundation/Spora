Latest online documentation available at: https://tondi.aspectron.org/docs/
...
- Merge with Rusty Tondi master (0.13.6)
...
- Rename `tondi-beacon` app to `tondi-resolver`
...
- Introduced `Beacon` class that provides connectivity to the community-operated public node infrastructure (backed by `tondi-beacon` load balancer & node status monitor)
...
- Changed documentation structure to use `typedoc` available as a part of redistributables or online at https://tondi.aspectron.org/docs/
