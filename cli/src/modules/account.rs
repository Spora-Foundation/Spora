        ...
        match action.as_str() {
            ...
            "import" => {
                if argv.is_empty() {
                    ...
                    ctx.term().help(
                        &[
                            ("account import mnemonic bip32", "Import Bip32 mnemonics used by sporawallet."),
                            (
        ...
        ctx.term().help(
            &[
                ...
                (
                    ...
                    "Import accounts from a private key using mnemonic data. Use 'account import' for additional help.",
                ),
                ("name <name>", "Name or rename the selected account (use 'remove' to remove the name"),
                ("scan [<derivations>] or scan [<start>] [<derivations>]", "Scan extended address derivation chain"),
                ("sweep [<derivations>] or sweep [<start>] [<derivations>]", "Sweep extended address derivation chain"),
