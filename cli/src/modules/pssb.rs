            ...
            "script" => {
                ...
                match subcommand.as_str() {
                    ...
                    "unlock" => {
                        ...
                        // Get locked cell set.
                        let spend_cells: Vec<spora_rpc_core::RpcCellsByAddressesEntry> =
                            ctx.wallet().rpc_api().get_cells_by_addresses(vec![script_p2sh.clone()]).await?;
                        let _priority_fee_sau = try_parse_optional_spora_as_sau_i64(argv.first())?.unwrap_or(0) as u64;

                        if spend_cells.is_empty() {
                            twarnln!(ctx, "No locked cell set found.");
                            return Ok(());
                        }
                        ...
                        tprintln!(
                            ctx,
                            "{} locked cell{} found with total amount of {} SPORA",
                            spend_cells.len(),
                            if spend_cells.len() == 1 { "" } else { "s" },
                            sau_to_spora(total_locked_sau)
                        );

                        // Sweep cell set.
                        match unlock_cells_as_pssb(references, &receive_address, script_sig, _priority_fee_sau as u64) {
                            Ok(pssb) => {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<SporaCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                ("pssb script unlock <payload> <fee>", "Generate a PSSB to unlock cells one by one from given P2SH payload. Fee amount will be applied to every spent cell, meaning every transaction. Optional public key placeholder in payload: {{pubkey}}"),
