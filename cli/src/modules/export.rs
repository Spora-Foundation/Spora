use crate::imports::*;
use tondi_wallet_core::account::{multisig::MultiSig, Account, BIP32_ACCOUNT_KIND, MULTISIG_ACCOUNT_KIND};
...
async fn export_multisig_account(ctx: Arc<TondiCli>, account: Arc<MultiSig>) -> Result<()> {
    match &account.prv_key_data_ids() {
        ...
        Some(prv_key_data_ids) => {
            ...
            for (id, prv_key_data_id) in prv_key_data_ids.iter().enumerate() {
                ...
                let xpub_key: tondi_bip32::ExtendedPublicKey<tondi_bip32::secp256k1::PublicKey> =
...
async fn export_single_key_account(ctx: Arc<TondiCli>, account: Arc<dyn Account>) -> Result<()> {
