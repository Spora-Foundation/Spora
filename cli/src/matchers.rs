...
pub fn register_link_matchers(cli: &Arc<TondiCli>) -> Result<()> {
    if !is_wasm() {
        return Ok(());
    }
    ...
    // addresses (open,copy) https://explorer.tondi.org/addresses/
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
        &js_sys::RegExp::new(r"(tondi|tonditest):\S+", "i"),
        Arc::new(Box::new(move |modifiers, uri| {
            if modifiers.ctrl || modifiers.meta {
                if uri.starts_with("tonditest") {
                    ...
                    let url = format!("https://explorer.tondi.org/addresses/{uri}");
                    if is_nw() {
                        nw_sys::shell::open_external(&url);
                    } else {
                        link::open(&url);
    ...
    // blocks (open,copy) https://explorer.tondi.org/blocks/
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                nw_sys::shell::open_external(&format!("https://explorer.tondi.org/blocks/{uri}"));
            } else {
    ...
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                nw_sys::shell::open_external(&format!("https://explorer.tondi.org/txs/{uri}"));
            } else {
                write_to_clipboard(&cli_, uri.to_string().as_str());
            }
        })),
...
fn write_to_clipboard(cli: &Arc<TondiCli>, text: &str) {
    if is_nw() {
