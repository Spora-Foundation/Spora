...
pub fn register_link_matchers(cli: &Arc<SporaCli>) -> Result<()> {
    if !is_wasm() {
        return Ok(());
    }
    ...
    // blocks (open,copy) https://explorer.spora.org/blocks/
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                nw_sys::shell::open_external(&format!("https://explorer.spora.org/blocks/{uri}"));
            } else {
    ...
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                nw_sys::shell::open_external(&format!("https://explorer.spora.org/txs/{uri}"));
            } else {
                write_to_clipboard(&cli_, uri.to_string().as_str());
            }
        })),
...
fn write_to_clipboard(cli: &Arc<SporaCli>, text: &str) {
    if is_nw() {
