        ...
        match transaction_data {
            ...
            | TransactionData::Change { utxo_entries, aggregate_input_value, .. } => {
                let aggregate_input_value =
                    transaction_type.style_with_sign(sau_to_tondi_string(*aggregate_input_value).as_str(), history);
                ...
                if include_utxos {
                    for utxo_entry in utxo_entries {
                        ...
                        let amount = transaction_type.style_with_sign(sau_to_tondi_string(utxo_entry.amount).as_str(), history);
            ...
            | TransactionData::TransferOutgoing { fees, aggregate_input_value, transaction, payment_value, change_value, .. } => {
                if let Some(payment_value) = payment_value {
                    lines.push(format!(
                        ...
                        "",
                        style(sau_to_tondi_string(*payment_value)).red(),
                        style(sau_to_tondi_string(*aggregate_input_value)).blue(),
                        style(sau_to_tondi_string(*fees)).red(),
                        style(sau_to_tondi_string(*change_value)).green(),
