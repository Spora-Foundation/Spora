...
pub fn register(ctx: FlowContext, router: Arc<Router>) -> Vec<Box<dyn Flow>> {
    ...
    let flows: Vec<Box<dyn Flow>> = vec![
        ...
        Box::new(HandleRelayInvsFlow::new(
            ...
            SharedIncomingRoute::new(
                router.subscribe_with_capacity(vec![P2pMessagePayloadType::InvRelayBlock], ctx.block_invs_channel_size()),
            ),
            router.subscribe(vec![P2pMessagePayloadType::Block, P2pMessagePayloadType::BlockLocator]),
            ibd_sender,
        )),
    ...
    // We do not register the below two messages because go-spora no longer serves them.
