use crate::{convert::error::ConversionError, core::peer::PeerKey, P2pMessagePayloadType};
...
pub enum ProtocolError {
    ...
    UnexpectedMessage(&'static str, Option<P2pMessagePayloadType>),
    ...
    IncomingRouteCapacityReached(P2pMessagePayloadType, String),
    ...
    NoRouteForMessageType(P2pMessagePayloadType),
...
impl ProtocolError {
    ...
    pub fn from_reject_message(reason: String) -> Self {
        if reason == LOOPBACK_CONNECTION_MESSAGE || reason == DUPLICATE_CONNECTION_MESSAGE {
            ...
            let hint = "Hint: If this error persists, it might be due to the other peer having pruned block data after syncing headers and cells. In such a case, you may need to reset the database.";
...
/// Wraps an inner payload message into a valid `P2pMessage`.
/// Usage:
/// ```ignore
...
macro_rules! make_message {
    ...
    ($pattern:path, $msg:expr, $response_id:expr, $request_id: expr) => {{
        $crate::pb::P2pMessage { payload: Some($pattern($msg)), response_id: $response_id, request_id: $request_id }
    }};
}
...
macro_rules! make_response {
    ($pattern:path, $msg:expr, $response_id:expr) => {{
        $crate::pb::P2pMessage { payload: Some($pattern($msg)), response_id: $response_id, request_id: 0 }
    }};
}
...
macro_rules! make_request {
    ($pattern:path, $msg:expr, $request_id:expr) => {{
        $crate::pb::P2pMessage { payload: Some($pattern($msg)), response_id: 0, request_id: $request_id }
    }};
}

/// Macro to extract a specific payload type from an `Option<pb::P2pMessage>`.
/// Usage:
/// ```ignore
...
/// Macro to await a channel `Receiver<pb::P2pMessage>::recv` call with a default/specified timeout and expect a specific payload type.
/// Usage:
/// ```ignore
...
/// Macro to indefinitely await a channel `Receiver<pb::P2pMessage>::recv` call and expect a specific payload type (without a timeout).
/// Usage:
/// ```ignore
