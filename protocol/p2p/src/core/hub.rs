use crate::{common::ProtocolError, pb::P2pMessage, ConnectionInitializer, Peer, Router};
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: P2pMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast_to_some_peers(&self, msg: P2pMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<P2pMessage>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self.peers.read().values().cloned().collect::<Vec<_>>();
